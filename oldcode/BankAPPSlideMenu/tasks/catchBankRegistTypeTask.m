...
@implementation catchBankRegistTypeTask
...
- (void)process {
    @try {
        NSString *url = [[App sharedInstance].cfg urlcatchBankRegistType];
        self.request = [[HttpRequest alloc] initWithDelegate:self];
//        [self.request requestPost:self.postDict withURL:url]; junyi.zhu 2014.03.09
        [self.request requestPostwithURL:url];
    }
