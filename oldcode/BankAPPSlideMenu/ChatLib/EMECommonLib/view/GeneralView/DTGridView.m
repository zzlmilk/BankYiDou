...
- (BOOL)cellInfoShouldBeOnShow:(NSObject<EMEGridViewCellInfoProtocol> *)info {
	...
	//NIF_TRACE(@"%@ NO: %@", NSStringFromSelector(_cmd), NSStringFromCGRect(info.frame));
...
- (void)checkViews {
  ...
  //  NIF_TRACE(@"isGoingUp: %i, isGoingDown: %i, co.y: %f, old.y: %f", isGoingUp, isGoingDown, self.contentOffset.y, oldContentOffset.y);
