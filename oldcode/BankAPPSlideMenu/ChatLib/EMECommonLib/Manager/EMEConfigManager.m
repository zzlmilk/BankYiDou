...
static EMEConfigManager *shareConfigManager = nil;
@implementation EMEConfigManager

+ (EMEConfigManager *)shareConfigManager {
	@synchronized(self) {
		if (shareConfigManager == nil) {
			shareConfigManager = [[EMEConfigManager alloc] init];
		}
...
-(NSDictionary *)getConfigDic{
    ...
    if (dic) {
        return [NSString parseJSONData:[dic dataUsingEncoding:NSUTF8StringEncoding]];
    }
    return nil;
}
