...
#define  NoticeCountKey [NSString stringWithFormat:@"%@_notice",self.user.userName]
#define  MessageNoticeForGroupSwithKey @"MessageNoticeForGroupSwithKey"
const NSString* c_my_account_archive_file_name = @"adprojectAccount.archive";
...
-(BOOL)can_auto_login
{
    if (self.user.userName!= nil && self.user.userPassword !=nil ) {
        return YES;
    }
    return NO;
}
...
- (void) init_singleton_from_disk
{
    ...
    NIF_INFO("check point 003, s_user=%@  %@",((User*)[NSKeyedUnarchiver unarchiveObjectWithFile:[[CommonUtils GetDocumentsPath] stringByAppendingPathComponent:(NSString*)c_my_account_archive_file_name]]).userName ,[s_user description] );
...
-(void)addNoticeCountWithMusicHints:(BOOL)isPlayMusicNotice SenderUID:(NSString*)senderUID
{
    ...
    if (isPlayMusicNotice) {
        ...
        NSNumber *newMessageVoiceNotice = [userDefault objectForKey:EME_NEWMESSAGE_VOICE_USERDEFAULT_KEY];
        if (![newMessageVoiceNotice boolValue]) {
            [[AudioPlayerManager shareInstance] player:@"message_notice.mp3"];
        }
        sleep(0.005);
        NSNumber *newMessageShakeNotice =[userDefault objectForKey:EME_NEWMESSAGE_SHAKE_USERDEFAULT_KEY];
     ...
     *2. 开始计数器
     */
    NSNumber* countNotice = [userDefault objectForKey:NoticeCountKey];
    if (countNotice == nil) {
    ...
    [userDefault setObject:countNotice forKey:NoticeCountKey];
...
{
    NSUserDefaults* userDefault = [NSUserDefaults standardUserDefaults];
    [userDefault setObject:[NSNumber numberWithInt:0] forKey:NoticeCountKey];
    [userDefault synchronize];
...
-(void)removeNoticeCountWithCount:(NSInteger)removeCount
{
 ...
 NSInteger leaveCount =    [self getNoticeCount] - removeCount ;
    ...
    NSUserDefaults* userDefault = [NSUserDefaults standardUserDefaults];
    [userDefault setObject:[NSNumber numberWithInt:leaveCount] forKey:NoticeCountKey];
    [userDefault synchronize];
...
//        [[NSNotificationCenter defaultCenter] postNotificationName:@"UNREAD" object:@{@"Chatid":senderUID,@"UnreadCount":[NSNumber numberWithInt:recentcontactsEntity.unReadMessagesCount]}];
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HaveNewMessageNotice object:[NSDictionary dictionaryWithObject:countNotice forKey:@"notice"]];
    }
}
...
{
    BOOL isOff= NO;
    NSSet* groupMessageSwithSet = [[NSUserDefaults standardUserDefaults] objectForKey:MessageNoticeForGroupSwithKey];
...
{
    if (groupId) {
         NSMutableSet* groupMessageSwithSet  = [NSMutableSet setWithArray:[[NSUserDefaults standardUserDefaults] objectForKey:MessageNoticeForGroupSwithKey]];
        [groupMessageSwithSet  addObject:groupId];
        [[NSUserDefaults standardUserDefaults] setObject:[groupMessageSwithSet allObjects]  forKey:MessageNoticeForGroupSwithKey];
