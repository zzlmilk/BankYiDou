...
-(void)registerCache:(BOOL)isCache MusicFileNameForKey:(NSString*)musicFileNameForKey CacheDirectory:(NSString*)cacheDirectory
{
    ...
    if(musicFileNameForKey){
        self.musicFileNameForKey = musicFileNameForKey;
         ...
         NIF_ALLINFO(@"保存的临时名称为: %@ , 请设置文件名字。",self.musicFileNameForKey);
...
{
    [self stop];
    NIF_ALLINFO(@"开始播放 录音的目录:%@",[self getMusicPathURL]);
    [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error{
    NIF_ALLINFO(@"An error happened :%@",error);
...
{
    NIF_ALLINFO(@"下载成功  ");
    NIF_ALLINFO(@"保存的目录:%@",[self getMusicPath]);
...
-(NSString*)renameMusicFileWithMusiceType:(SupportMusicType)supportMusicType
{
    ...
    if (error) {
        NIF_ALLINFO(@"重新命名失败 ：%@",[error description]);
        return OldPath;
    }else{
        return newPath;
    }
