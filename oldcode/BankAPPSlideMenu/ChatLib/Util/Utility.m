...
{
    UIView *view=[Utility getSubView:searchBar withClassName:NSClassFromString(@"UISearchBarBackground")];
    UITextField *textField=(UITextField *)[Utility getSubView:searchBar withClassName:NSClassFromString(@"UISearchBarTextField")];
    if (view!=nil&&textField!=nil) {
        UIImageView *imageView=[[UIImageView alloc] initWithImage:[Utility createUIImageWithSize:CGSizeMake(320, 44) imageColor:color]];
...
//    NSInteger pos=[val indexOf:delimiter];
    NSInteger pos=[Utility string:val indexOf:delimiter];
...
//        pos=[val indexOf:delimiter];
        pos=[Utility string:val indexOf:delimiter];
    }
    if ([val length]>0) {
        [reVal addObject:val];
    }
...
UIColor *dottedLine(NSUInteger frontColorARGB,NSUInteger backgroundColorARGB,CGFloat space,CGFloat width,CGFloat length)
{
    ...
    UIImage *image=[Utility createUIImageWithSize:CGSizeMake(length, width) imageColor:backgroundColor];
...
{
    UIColor *backgroundColor=colorWithHexARGB(backgroundColorARGB);
    UIImage *image=[Utility createUIImageWithSize:CGSizeMake(length, width+width) imageColor:backgroundColor];
    UIImageView *imageView=[[UIImageView alloc] initWithImage:image];
...
{
    CGFloat x=([Screen applicationFrameWidth]-[view frame].size.width)/2;
    CGFloat y=([Screen applicationFrameHeight]-[view frame].size.height)/2;
...
{
    UIView *subview=[Utility placeImageInView:view point:point imageName:imageName];
    return subview;
}
...
{
    UIView *subview=[Utility placeImageInView:view insets:insets imageName:imageName];
    return subview;
}
...
{
    UIButton *button=[Utility buttonWithFrame:frame backgoundColorARGB:colorARGB fileLayer:fileLayer fileButtonUp:fileButtonImage fileButtonDown:fileButtonImage];
...
+(void)alert:(NSString *)content{
    UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"系统提示" message:content delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
    [alert show];
